...
def save_face_embedding(db_path, name, model, detector, embedding, embedding_normalized, image_count,
                        first_name=None, last_name=None, employee_id=None, email=None,
                        profile_image_path=None, images_directory=None):
...
class GuidedEnrollmentCapture:
    ...
    def _process_enrollment(self):
        ...
        if not self.employee_id:
            ...
            self.enrollment_result = "❌ No employee ID provided"
            return
        ...
        # Create images directory organized by employee ID
        images_dir = os.path.join(OUTPUT_DIR, "images", self.employee_id)
        os.makedirs(images_dir, exist_ok=True)
        logger.info(f"   Images directory: {images_dir}")
        
        # Profile image path (128x128 thumbnail of front-facing capture)
        ...
        for i, frame in enumerate(self.captured_frames):
            ...
            try:
                # Save the captured image with pose name
                pose_name = pose_names[i] if i < len(pose_names) else f"pose_{i+1}"
                image_path = os.path.join(images_dir, f"{pose_name}.jpg")
                # Raw frame is already BGR from OpenCV
                cv2.imwrite(image_path, frame)
                logger.info(f"      Saved image: {image_path}")
                
                # Create 128x128 profile thumbnail from the FIRST (front-facing) capture
                if i == 0:
                    # Detect face region for better thumbnail
                    try:
                        from deepface import DeepFace
                        ...
                        if faces and len(faces) > 0:
                            ...
                            # face_img is normalized 0-1 float, convert to uint8
                            face_img = (face_img * 255).astype(np.uint8)
                            # Convert RGB to BGR for OpenCV
                            face_img = cv2.cvtColor(face_img, cv2.COLOR_RGB2BGR)
                            # Resize to 128x128
                            thumbnail = cv2.resize(face_img, (128, 128), interpolation=cv2.INTER_AREA)
                        else:
                            # Fallback: just resize the whole frame
                            thumbnail = cv2.resize(frame, (128, 128), interpolation=cv2.INTER_AREA)
                    except Exception as e:
                        logger.warning(f"      ⚠️ Face extraction failed for thumbnail: {e}")
                        ...
                        thumbnail = cv2.resize(cropped, (128, 128), interpolation=cv2.INTER_AREA)
        ...
        if len(embeddings) >= 2:
            ...
            logger.info(f"   Employee ID: {self.employee_id}")
            logger.info(f"   Profile image: {profile_image_path}")
            logger.info(f"   Embedding shape: {master_embedding.shape}")
            self.enrollment_result = f"✅ SUCCESS! {self.first_name} {self.last_name} (ID: {self.employee_id}) enrolled with {len(embeddings)} photos."
        else:
...
def start_enrollment(first_name, last_name, employee_id, email):
    ...
    capture_system.set_user_info(first_name, last_name, employee_id, email)
...
with create_blocks() as demo:
    ...
    start_btn.click(
        fn=start_enrollment,
        inputs=[first_name_input, last_name_input, employee_id_input, email_input],
        outputs=[welcome_screen, camera_screen, error_msg]
    )
    
    # Allow Enter key on any input field to start
