...
def save_face_to_db(db_path: str, name: str, model: str, embedding: np.ndarray, 
                    image_count: int, detector: str = None, first_name: str = None,
                    last_name: str = None, employee_id: str = None, email: str = None,
                    profile_image_path: str = None, images_directory: str = None) -> bool:
...
def list_faces_from_db(db_path: str) -> List[Dict]:
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT name, first_name, last_name, employee_id, email, 
               model, detector, image_count, profile_image_path, images_directory, enrolled_at 
        FROM faces
    """)
...
class GradioInterface:
    ...
    def _get_employee_cards_html(self):
        """Generate HTML for employee cards display."""
        enrolled = self.system.list_enrolled()
        if not enrolled:
            return """
            <div style="text-align: center; padding: 40px; color: #888;">
                <h3>📋 No Employees Enrolled</h3>
                <p>Use the facial_enrollment.py script to enroll new employees.</p>
            </div>
            """
        
        cards_html = '<div style="display: flex; flex-wrap: wrap; gap: 20px; padding: 20px;">'
        ...
        for entry in enrolled:
            ...
            profile_image_path = entry.get('profile_image_path') or ''
            enrolled_at = entry.get('enrolled_at') or 'Unknown'
            
            # Format the enrolled date
            if enrolled_at and enrolled_at != 'Unknown':
                try:
                    from datetime import datetime
                    dt = datetime.fromisoformat(enrolled_at)
                    enrolled_at = dt.strftime('%Y-%m-%d %H:%M')
                except:
                    pass
            
            # Check if profile image exists and create base64 data URI
            if profile_image_path and os.path.exists(profile_image_path):
                ...
                # Placeholder image (gray square with initials)
                initials = (first_name[:1] + last_name[:1]).upper() if first_name or last_name else '?'
                img_src = f"data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' width='128' height='128'><rect fill='%23444' width='128' height='128'/><text x='50%' y='50%' dominant-baseline='middle' text-anchor='middle' fill='white' font-size='48'>{initials}</text></svg>"
            ...
            ">
                <div style="display: flex; align-items: center; gap: 15px; margin-bottom: 15px;">
                    <img src="{img_src}" alt="Profile" style="
                        width: 80px;
                        height: 80px;
                        border-radius: 50%;
                        object-fit: cover;
                    ...
                    "/>
                    <div>
                        <h3 style="margin: 0; color: #fff; font-size: 1.2em;">{first_name} {last_name}</h3>
                        <p style="margin: 5px 0 0 0; color: #aad4f5; font-size: 0.9em;">ID: {employee_id}</p>
                    </div>
                </div>
                <div style="border-top: 1px solid #3d7ab8; padding-top: 12px;">
                    <p style="margin: 5px 0; color: #cce5ff; font-size: 0.85em;">
                        <strong style="color: #5bc0de;">📧 Email:</strong><br/>
                        <span style="word-break: break-all;">{email}</span>
                    </p>
                    <p style="margin: 5px 0; color: #cce5ff; font-size: 0.85em;">
                        <strong style="color: #5bc0de;">📅 Enrolled:</strong> {enrolled_at}
                    </p>
                </div>
            </div>
            '''
    ...
    def _get_employee_names_for_dropdown(self):
        """Get list of employee names for the delete dropdown."""
        enrolled = self.system.list_enrolled()
        if not enrolled:
            return []
        return [entry['name'] for entry in enrolled]
    
    def _get_employee_list_html(self):
        """Generate HTML for employee list view (clickable names)."""
        enrolled = self.system.list_enrolled()
        if not enrolled:
            return """
            <div style="text-align: center; padding: 40px; color: #888;">
            ...
            <table style="width: 100%; border-collapse: collapse;">
                <thead>
                    <tr style="background: #1e3a5f; color: white;">
                        <th style="padding: 12px; text-align: left; border-bottom: 2px solid #3d7ab8;">Name</th>
                        <th style="padding: 12px; text-align: left; border-bottom: 2px solid #3d7ab8;">Employee ID</th>
                        <th style="padding: 12px; text-align: left; border-bottom: 2px solid #3d7ab8;">Email</th>
                        <th style="padding: 12px; text-align: left; border-bottom: 2px solid #3d7ab8;">Enrolled</th>
                    </tr>
                </thead>
                <tbody>
        '''
        ...
        for i, entry in enumerate(enrolled):
            first_name = entry.get('first_name') or ''
            last_name = entry.get('last_name') or ''
            full_name = f"{first_name} {last_name}".strip() or entry.get('name', 'Unknown')
            employee_id = entry.get('employee_id') or 'N/A'
            email = entry.get('email') or 'N/A'
            enrolled_at = entry.get('enrolled_at') or 'Unknown'
            ...
            if enrolled_at and enrolled_at != 'Unknown':
                try:
                    from datetime import datetime
                    dt = datetime.fromisoformat(enrolled_at)
                    enrolled_at = dt.strftime('%Y-%m-%d')
                except:
                    pass
            
            row_bg = '#2d5a87' if i % 2 == 0 else '#1e3a5f'
            
            list_html += f'''
                <tr style="background: {row_bg}; cursor: pointer;" 
                    onmouseover="this.style.background='#3d7ab8'" 
                    onmouseout="this.style.background='{row_bg}'">
                    <td style="padding: 12px; color: #fff; font-weight: bold;">{full_name}</td>
                    <td style="padding: 12px; color: #cce5ff;">{employee_id}</td>
                    <td style="padding: 12px; color: #cce5ff;">{email}</td>
                    <td style="padding: 12px; color: #aad4f5;">{enrolled_at}</td>
                </tr>
            '''
        ...
        list_html += '''
            ...
            <p style="color: #888; font-size: 0.9em; margin-top: 15px; text-align: center;">
                👆 Select an employee from the dropdown below to view details
            </p>
        </div>
        '''
        return list_html
    
    def _get_employee_details(self, selected_name):
        """Get detailed view for a selected employee."""
        if not selected_name:
            return None, """
            <div style="text-align: center; padding: 40px; color: #888;">
                <p>👆 Select an employee from the dropdown to view details</p>
            </div>
            """
        ...
        if not employee:
            ...
            <div style="text-align: center; padding: 40px; color: #ff6b6b;">
                <p>❌ Employee '{selected_name}' not found</p>
            </div>
            """
        
        first_name = employee.get('first_name') or ''
        last_name = employee.get('last_name') or ''
        employee_id = employee.get('employee_id') or 'N/A'
        email = employee.get('email') or 'N/A'
        model = employee.get('model') or 'Unknown'
        image_count = employee.get('image_count') or 0
        profile_image_path = employee.get('profile_image_path') or ''
        images_directory = employee.get('images_directory') or ''
        enrolled_at = employee.get('enrolled_at') or 'Unknown'
        ...
        if enrolled_at and enrolled_at != 'Unknown':
            try:
                from datetime import datetime
                dt = datetime.fromisoformat(enrolled_at)
                enrolled_at = dt.strftime('%Y-%m-%d %H:%M:%S')
            except:
                pass
        ...
        <div style="
            ...
            border: 1px solid #3d7ab8;
        ">
            <h2 style="color: #fff; margin-top: 0; border-bottom: 2px solid #5bc0de; padding-bottom: 10px;">
                👤 {first_name} {last_name}
            </h2>
            
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-top: 20px;">
                <div>
                    <p style="margin: 10px 0; color: #cce5ff;">
                        <strong style="color: #5bc0de;">🆔 Employee ID:</strong><br/>
                        <span style="font-size: 1.1em; color: #fff;">{employee_id}</span>
                    </p>
                    <p style="margin: 10px 0; color: #cce5ff;">
                        <strong style="color: #5bc0de;">📧 Email:</strong><br/>
                        <span style="font-size: 1.1em; color: #fff;">{email}</span>
                    </p>
                </div>
                <div>
                    <p style="margin: 10px 0; color: #cce5ff;">
                        <strong style="color: #5bc0de;">🤖 Model:</strong><br/>
                        <span style="color: #fff;">{model}</span>
                    </p>
                    <p style="margin: 10px 0; color: #cce5ff;">
                        <strong style="color: #5bc0de;">📷 Images Captured:</strong><br/>
                        <span style="color: #fff;">{image_count}</span>
                    </p>
                </div>
            </div>
            
            <div style="margin-top: 15px; padding-top: 15px; border-top: 1px solid #3d7ab8;">
                <p style="margin: 10px 0; color: #cce5ff;">
                    <strong style="color: #5bc0de;">📅 Enrolled:</strong> {enrolled_at}
                </p>
                <p style="margin: 10px 0; color: #cce5ff;">
                    <strong style="color: #5bc0de;">📁 Images Directory:</strong><br/>
                    <code style="background: #0d1f30; padding: 5px 10px; border-radius: 4px; color: #aad4f5; font-size: 0.9em;">
                        {images_directory or 'Not available'}
                    </code>
                </p>
            </div>
        </div>
        ...
        return profile_img, details_html
    
    def _get_employee_dropdown_choices(self):
        """Get choices for employee selection dropdown."""
        enrolled = self.system.list_enrolled()
        if not enrolled:
            return []
        choices = []
        for entry in enrolled:
            first_name = entry.get('first_name') or ''
            last_name = entry.get('last_name') or ''
            display_name = f"{first_name} {last_name}".strip() or entry.get('name', 'Unknown')
            employee_id = entry.get('employee_id') or ''
            label = f"{display_name} ({employee_id})" if employee_id else display_name
            choices.append((label, entry.get('name')))
        return choices
    ...
    def build(self):
        ...
        with gr.Blocks() as demo:
            ...
            **Current Model:** `{self.system.model_name}` | **Detector:** `{self.system.detector}` | **Threshold:** `{self.system.threshold}`
            ...
            # Tab 3: Enrolled Employees
            with gr.Tab("👥 Enrolled Employees"):
                gr.Markdown("""
                ### Enrolled Employees
                View all employees enrolled in the facial recognition system.
                
                > **Note:** To enroll new employees, use the dedicated enrollment kiosk: `python facial_enrollment.py`
                """)
                
                with gr.Row():
                    view_mode = gr.Radio(
                        choices=["📇 Card View", "📋 List View"],
                        value="📇 Card View",
                        label="Display Mode",
                        interactive=True
                    )
                ...
                # Card View Container
                with gr.Column(visible=True) as card_view_container:
                    employee_cards = gr.HTML(
                        value=self._get_employee_cards_html(),
                        label="Enrolled Employees"
                    )
                
                # List View Container
                with gr.Column(visible=False) as list_view_container:
                    employee_list_html = gr.HTML(
                        value=self._get_employee_list_html(),
                        label="Employee List"
                    )
                    
                    gr.Markdown("---")
                    gr.Markdown("### 👤 Employee Details")
                    
                    # Dropdown to select employee
                    employee_selector = gr.Dropdown(
                        choices=self._get_employee_dropdown_choices(),
                        label="Select Employee to View Details",
                        interactive=True,
                ...
                gr.Markdown("---")
                gr.Markdown("### 🗑️ Manage Employees")
                with gr.Row():
                    with gr.Column(scale=2):
                        delete_name = gr.Textbox(
                            label="Employee Name to Delete", 
                            placeholder="Enter full name (e.g., John_Doe)"
                        )
                    with gr.Column(scale=1):
                ...
                # Refresh handler - updates both views and dropdown
                def refresh_all():
                    cards = self._get_employee_cards_html()
                    list_html = self._get_employee_list_html()
                    dropdown_choices = self._get_employee_dropdown_choices()
                    return cards, list_html, gr.update(choices=dropdown_choices, value=None), None, """
                        <div style="text-align: center; padding: 40px; color: #888;">
                            <p>👆 Select an employee from the dropdown to view details</p>
                        </div>
                    """
                
                # Delete handler - refreshes both views
                def delete_and_refresh_all(name):
                    result = self._delete_face(name)
                    cards = self._get_employee_cards_html()
                    list_html = self._get_employee_list_html()
                    dropdown_choices = self._get_employee_dropdown_choices()
                    return result, cards, list_html, gr.update(choices=dropdown_choices, value=None)
                
                # Event handlers
                view_mode.change(
                    fn=toggle_view,
                    inputs=[view_mode],
                    outputs=[card_view_container, list_view_container]
                )
                
                refresh_btn.click(
                    fn=refresh_all,
                    outputs=[employee_cards, employee_list_html, employee_selector, profile_image_display, employee_details_html]
                )
                
                employee_selector.change(
                    fn=self._get_employee_details,
                    inputs=[employee_selector],
                    outputs=[profile_image_display, employee_details_html]
                )
                
                delete_btn.click(
                    fn=delete_and_refresh_all,
                    inputs=[delete_name],
                    outputs=[delete_output, employee_cards, employee_list_html, employee_selector]
                )
